...
function parseAssetList(rawAssets) {
  ...
  return list
    ...
    .map((item, index) => {
      ...
      const placement = String(item?.placement || 'all-mascot').trim().toLowerCase();
...
const MASCOT_SLIDES = new Set(['cover', 'meet-buddy', 'example-interaction', 'closing']);

export function findAssetForSlide(slide, deckData) {
  if (slide?.imageAsset?.dataUrl) return slide.imageAsset;

  const assets = parseAssetList(deckData?.content?.characterAssets || deckData?.project?.characterAssets || []);
