...
function renderSplit(slide, slideInfo, model, step, lines, ratio = '4:3') {
  const { deckTheme: theme } = model;
  bg(slide, theme.backgroundColor);
  addTop(slide, slideInfo, theme, step);
...
function renderAnalytics(slide, slideInfo, model, step) {
  const lines = [slideInfo.description, ...(slideInfo.bullets || [])].filter(Boolean);
...
function renderClosing(slide, slideInfo, model, step) {
  ...
  slide.addText(`${project.contactName}\n${project.contactEmail}\n${project.contactPhone}`, {
    x: 1.0,
    y: 4.2,
    w: 4.6,
    h: 1.2,
  ...
  slides.forEach((slideInfo, idx) => {
    ...
    switch (slideInfo.type) {
      ...
      case 'problem':
        ...
        break;
      case 'opportunity':
        renderSplit(slide, slideInfo, model, step, slideInfo.points || [], '4:3');
        break;
      case 'solution':
      ...
      case 'experience-concept':
        ...
        break;
      case 'chat-flow':
        renderSplit(slide, slideInfo, model, step, slideInfo.steps || [], '4:3');
        break;
