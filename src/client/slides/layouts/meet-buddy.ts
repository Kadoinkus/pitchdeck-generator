...
export function renderMeetBuddy(
	slide: SlideData,
	theme: ThemeData,
	deckData: DeckData,
): string {
    ...
    <article class="text-surface text-panel">
      ...
      <p class="paragraph" ${attrTarget(target, `${slide.title} description`)}>${
		esc(fitText(slide.description || '', 210))
	}</p>
      ...
      <div class="tone-list" ${attrTarget('toneSliders', `${slide.title} tone`)}>
        ${
		tone.map((item) =>
			`<div class="tone-row"><span>${esc(item.label || '')}</span><div class="tone-track"><i style="width:${
				Math.max(10, Math.min(100, Number(item.value) || 70))
			}%"></i></div></div>`
		).join('')
	}
      </div>
