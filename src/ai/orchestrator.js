...
import { localGenerateAutofill, localChatAssist } from './providers/local-provider.js';
import { openAIAutofill, openAIChatAssist } from './providers/openai-provider.js';

const EDITABLE_FIELDS = new Set(getEditableFieldDefinitions().map((field) => field.name));
...
function sanitizeChanges(changes) {
  ...
  return changes
    ...
    .filter((change) => change.field && EDITABLE_FIELDS.has(change.field) && change.value);
}
...
function resolveAiConfig(rawData = {}) {
  return {
    ...
    textApiKey: safeText(rawData.aiTextApiKey, process.env.OPENAI_API_KEY || ''),
    textBaseUrl: safeText(rawData.aiTextBaseUrl, process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1'),

    imageProvider: safeText(rawData.aiImageProvider, 'local').toLowerCase(),
    imageModel: safeText(rawData.aiImageModel, 'gpt-4.1-mini'),
    imageApiKey: safeText(rawData.aiImageApiKey, process.env.OPENAI_API_KEY || ''),
    imageBaseUrl: safeText(rawData.aiImageBaseUrl, process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1')
  };
}
...
export async function generateAutofill(rawData = {}) {
  ...
  if (config.textProvider === 'openai' && config.textApiKey) {
    try {
      ...
      console.error('OpenAI autofill failed, falling back to local engine:', error.message);
    }
  }
...
export async function runChatAssistant(rawData = {}, chatRequest = {}) {
  ...
  if (config.textProvider === 'openai' && config.textApiKey) {
    try {
      ...
      console.error('OpenAI chat assistant failed, falling back to local engine:', error.message);
    }
  }
