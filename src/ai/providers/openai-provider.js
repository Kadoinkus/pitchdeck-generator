...
async function callOpenAIChat({ apiKey, baseUrl, model, messages, temperature = 0.4 }) {
...
function sanitizeSuggestedChanges(changes) {
  ...
  return changes
    ...
    .filter((change) => change.field && ALLOWED_FIELDS.has(change.field) && change.value);
}
...
export async function openAIAutofill(rawData = {}, config = {}) {
  ...
  const includedSlides = availableSlides.filter((slide) => slide.included).map((slide) => slide.id);
  const fieldGuide = AUTOFILL_FIELDS.map((fieldName) => {
    const formatHint = MULTILINE_FIELDS.has(fieldName) ? 'multiline string' : 'single line string';
  ...
  const content = await callOpenAIChat({
    ...
    temperature: 0.45,
    messages: [
      { role: 'system', content: 'You are a proposal deck content generator. Always respond with strict JSON.' },
      { role: 'user', content: prompt }
    ]
  });

  const parsed = extractJsonObject(content);
  const prompts = Array.isArray(parsed?.imageDraft?.prompts) ? parsed.imageDraft.prompts : [];

  const sanitizedPrompts = prompts
    .map((item, index) => ({
      slideId: safeText(item?.slideId, includedSlides[index] || `slide-${index + 1}`),
      slideNumber: index + 1,
      slideTitle: safeText(item?.slideTitle, item?.slideId || `Slide ${index + 1}`),
  ...
  return {
    ...
    imageDraft: {
      ...
      combinedPromptText: sanitizedPrompts.map((item) => `Slide ${item.slideNumber} (${item.slideId}) :: ${item.prompt}`).join('\n')
    }
  };
}

export async function openAIChatAssist(rawData = {}, chatRequest = {}, config = {}) {
  ...
  const content = await callOpenAIChat({
    ...
    temperature: 0.55,
    messages: [
      { role: 'system', content: 'You are a deck editing copilot. Always return strict JSON only.' },
      { role: 'user', content: prompt }
    ]
  });
